...
function beautifulDays(i: number, j: number, k: number): number {
    ...
    let goodDay:number = 0
    
    for(let n = i ; n<=j ; n++){
        
        const reverseStr : string =n.toString().split("").reverse().join('')
        const reverseNum :number = parseInt(reverseStr,10)
        const num :number = (Math.abs(n-reverseNum))/k
